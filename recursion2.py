...
#  Description: More recursion practice with subsets
...
def splitArrayHelper(nums, index, sum1, sum2):
    if index>=len(nums):
        ...
        return splitArrayHelper(nums, index+1,sum1+nums[index-1],sum2) or splitArrayHelper(nums, index+1,sum1,sum2+nums[index-1])
...
def splitOdd10Helper(nums, index, sum1, sum2):
    if index>=len(nums):
        ...
        return splitOdd10Helper(nums, index+1,sum1+nums[index-1],sum2) or splitOdd10Helper(nums, index+1,sum1,sum2+nums[index-1])
...
def split53Helper(nums, index, sum1, sum2):
    if index>=len(nums):
        return sum1==sum2
    elif (nums[index] % 3 == 0) and (nums[index] % 5 != 0):
        return split53Helper(nums, index+1,sum1,sum2+nums[index])
    elif (nums[index] % 5 == 0) and (nums[index] % 3 != 0):
        return split53Helper(nums, index+1,sum1+nums[index],sum2)
    else:
        return split53Helper(nums, index+1,sum1+nums[index],sum2) or split53Helper(nums, index+1,sum1,sum2+nums[index])
